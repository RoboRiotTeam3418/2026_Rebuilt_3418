...
public class wiggleIntake extends Command {
  ...
  public wiggleIntake(intakeSubsystem intake) { // Constructor | Creates new intakeSubsystem Command
  ...
  public void execute() {
    ...
    */
    if (m_Intake.getPivotEncoderPos()>=SubsystemConstants.INTAKE_MAX_ANGLE_IN-.1&&spd>0) {
      spd = -.3;
    }
    if (m_Intake.getPivotEncoderPos()<=SubsystemConstants.INTAKE_MAX_ANGLE_OUT+.1&&spd<0) {
      spd = .3;
    }
    m_Intake.pivot(spd);
  ...
  public boolean isFinished() {
    return false;//(m_Intake.getPivotEncoderPos() >= SubsystemConstants.INTAKE_MAX_ANGLE_IN && spd>0)||(m_Intake.getPivotEncoderPos() <= SubsystemConstants.INTAKE_MAX_ANGLE_OUT && spd<0);
  }
}